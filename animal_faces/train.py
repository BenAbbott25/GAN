...
def train(image_size: int = 64, batch_size: int = 16, training_steps: int = 500, grayscale: bool = False):
    ...
    for (step, true_data) in enumerate(train_loader):
        if step >= training_steps or true_data.shape[0] != batch_size:
            break

        true_data = true_data.to(device)
        print(f'true_data: {true_data.shape}')
...
if __name__ == "__main__":
    plt.ion()
    train(image_size=64, batch_size=64, training_steps=5000, grayscale=True)
